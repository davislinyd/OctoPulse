# OctoPulse v3 Agent Rules

OctoPulse v3 uses `.octopulse` as the sole project progress source.

## Context boundary

- Before reading progress, run `octopulse context` from the current Git project.
- Read only `.octopulse` and lightweight Git facts for ordinary project status work.
...
- After non-trivial work, update `.octopulse` only when its factual status changes, then run `octopulse validate .octopulse`.

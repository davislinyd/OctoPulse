...
安裝最新 GitHub Release。`auto` 模式會依 Codex、Claude Code、Antigravity 的優先序只安裝一份已偵測的 global skill，避免同一個 loader 載入重複 skill：

```sh
curl -fsSL https://github.com/davislinyd/OctoPulse/releases/latest/download/install.sh | sh
```

只有在各平台不共用 skill loader 時，才安裝所有三種 adapter：

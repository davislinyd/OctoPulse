...
安裝最新 GitHub Release。Codex 與 Antigravity 共用 `~/.agents/skills`；`auto` 模式只會安裝一份已偵測的 global skill，避免同一個 loader 載入重複 skill。請選擇其一：
...
只有在需要同時支援 Claude Code 時，才在上述任一指令加上 `--agent all`；Codex 與 Antigravity 仍共用同一份 `~/.agents/skills/octopulse`：
...
完成 global skill 安裝後，在已納管專案中直接告訴 Agent：「使用 OctoPulse skill 取得目前專案狀態，完成這項非瑣碎工作後更新 pulse 與專案報告。」Codex、Claude Code 或 Antigravity 會先執行：
